...
class LocationController2: LocationController {
    var recentLoc: CLLocation?

    override func startWalkingProcess() {
        guard let bWalkingState = walkingController?.bWalkingState else {
            return
        }
    ...
    override func updateCurrLocation(_ locations: [CLLocation]) {
        let bWalkingState = walkingController?.bWalkingState ?? false;
