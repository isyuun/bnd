...
class MainViewController: LocationViewController {
    ...
    func test() {
        ...
        let directoryPath: URL = documentPath.appendingPathComponent("gpx")
        // 디렉토리에 만들 '파일이름.확장자' 설정 (경로에 파일의 새 경로 추가)
        let textPath: URL = directoryPath.appendingPathComponent("history.GPX")
...
//        if let data: Data = "안녕하세요.".data(using: String.Encoding.utf8) { // String to Data
        if let data: Data = getData().data(using: String.Encoding.utf8) { // String to Data
            do {
                try data.write(to: textPath) // 위 data를 파일에 쓰기
            } catch let e {
                print(e.localizedDescription)
            }
        }
...
//        bottomSheetVC.isDynamicHeight = true
    ...
    override func updateCurrLocation(_ locations: [CLLocation]) {
        ...
        WeatherAPI.weather(request: request) { weather, error in
            ...
            if let error = error {
                self.showSimpleAlert(title: "Weather fail", msg: error.localizedDescription)
            }
        }
    }
