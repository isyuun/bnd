...
class StoryModViewController: CommonPostViewController {
    ...
    private func setCurrSelPetData(_ data: Any) {
        guard let pets = data as? [Pet] else { return }
        ...
        if let dailyLifePetList = lifeViewData.dailyLifePetList {
            for i in 0..<dailyLifePetList.count {
                for j in 0..<pets.count {
                    if dailyLifePetList[i].ownrPetUnqNo == pets[j].ownrPetUnqNo {
        ...
        self.pets = arrCurrSelDailyLifePet
    ...
    private func getSelectedPets() -> [DailyLifePetList] {
        ...
        for i in 0..<itemSelected.count {
            if itemSelected[i] == false, let pets = self.pets {
                let deleteItem = DailyLifePetList(ownrPetUnqNo: pets[i].ownrPetUnqNo,
                                                  petNm: pets[i].petNm,
                                                  bwlMvmNmtm: pets[i].bwlMvmNmtm,
                                                  urineNmtm: pets[i].urineNmtm,
                                                  relmIndctNmtm: pets[i].relmIndctNmtm,
                                                  stdgUmdNm: pets[i].stdgUmdNm,
                                                  age: pets[i].age,
                                                  schUnqNo: pets[i].schUnqNo,
                                                  petImg: pets[i].petImg,
                                                  petTypCd: pets[i].petTypCd,
                                                  rowState: "D")
    ...
    private func getSelectedDailyLifeCnt() -> Int {
        ...
        let pets = getSelectedDailyLife()
        for i in 0..<pets.count {
            if pets[i].rowState != "D" {
                ret += 1
            }
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == cv_currSelPet {
            ...
            if let pet = self.pets?[indexPath.row] {
                Global2.setPetImage(imageView: cell.ivProf, petTypCd: pet.petTypCd, petImgAddr: pet.petImg)
