...
class CommunityCommentView : UIView {
    ...
    func initialize(_isReplyMode: Bool, _data: CommentData, _atchPath: String, _rootView: UIView, _delegate: CommunityCommentViewDelegate) {
        ...
        if let petImg = data.comment.petImg {
            iv_prof.af.setImage(
                withURL: URL(string: String("\(atchPath)\(petImg)"))!,
                placeholderImage: UIImage(named: "profile_default")!,
                filter: AspectScaledToFillSizeFilter(size: iv_prof.frame.size)
            )
        } else {
            iv_prof.image = UIImage(named: "profile_default")!
            iv_prof.contentMode = .scaleAspectFill
        }
